...
export default async function StoreDetailPage(container, params) {
    ...
    try {
        ...
        // 使用后端返回的liked字段判断是否已点赞
        const hasLiked = storeDetails.liked === true;
        
        // 保存初始点赞数，以便更新UI
        let currentLikesCount = storeDetails.likes || 0;
        ...
        storeDetailContainer.innerHTML = `
            <div class="store-info">
                ...
                <div class="store-meta">
                    <span id="likes-count">👍 ${currentLikesCount} 人点赞</span> · 
        ...
        if (likeButton && isLoggedIn) {
            // 使用let而不是const，这样可以更新
            let isCurrentlyLiked = storeDetails.liked === true;
            let currentLikesCount = storeDetails.likes || 0;
            
            likeButton.addEventListener('click', async () => {
                try {
                    likeButton.disabled = true; // 防止连续点击
                    ...
                    if (isCurrentlyLiked) {
                        ...
                        // 更新点赞计数
                        currentLikesCount--;
                        likesCountElement.textContent = `👍 ${currentLikesCount} 人点赞`;
                    } else {
                        // 未点赞，添加点赞
    ...
    function renderComments(comments) {
        ...
        comments.forEach(comment => {
            ...
            // 检查当前用户是否已经点赞该评论 - 修改为使用comment.liked
            const hasLikedComment = comment.liked === true;
            ...
            if (likeButton && isLoggedIn) {
                // 使用let定义可变状态
                let isCommentLiked = hasLikedComment;
                ...
                likeButton.addEventListener('click', async () => {
                    try {
                        ...
                        if (isCommentLiked) {
                            // 已点赞，取消点赞
                            await api.unlikeComment(comment.id, storeId);
                            
                            // 更新状态和UI
                            isCommentLiked = false;
                            likeButton.classList.remove('liked');
                            likesCount = Math.max(0, likesCount - 1); // 确保不会出现负数
                            likeButton.innerHTML = `🤍 ${likesCount}`;
                        } else {
                            // 未点赞，添加点赞
                            await api.likeComment(comment.id, storeId, {
                                targetName: comment.content.substring(0, 20) || '评论',
                                targetId: comment.id,
                                targetType: 'comment'
                            });
                            ...
                            likeButton.classList.add('liked');
                            likesCount += 1;
                            likeButton.innerHTML = `❤️ ${likesCount}`;
                        }
                        
                        likeButton.disabled = false; // 恢复按钮状态
                    } catch (error) {
                        console.error('评论点赞操作失败:', error);
                        alert('点赞操作失败，请稍后重试');
                        likeButton.disabled = false; // 确保出错时也恢复按钮状态
                    }
                });
            }
            ...
            if (deleteButton) {
                deleteButton.addEventListener('click', async () => {
                    if (confirm('确定要删除这条评论吗？')) {
                        try {
                            // 修正参数顺序
                            await api.deleteComment(comment.id, storeId);
                            commentItem.remove();
